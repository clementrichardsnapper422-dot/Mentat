...
.good {{ color:#69e2af; }} .warn {{ color:#ffcc73; }} .bad {{ color:#ff8296; }}
.row {{ display:flex; justify-content:space-between; gap:16px; padding:9px 0; border-top:1px solid #242b45; }}
.row:first-of-type {{ border-top:0; }}
small {{ color:#9aa4c8; }}
button {{ border:0; border-radius:10px; padding:10px 14px; font-weight:700; cursor:pointer; background:#8177ff; color:white; }}
button.danger {{ background:#b84258; }} button.safe {{ background:#287b62; }}
...
@media(max-width:980px) {{ .card,.card.wide {{ grid-column:1/-1; }} }}
</style>
</head>
<body>
<header><div><h1>Mentat Control Center</h1><div class="sub">Release gates, spending authority, recovery, and execution evidence</div></div><button onclick="refresh()">Refresh</button></header>
<main>
...
async function refresh() {{ try {{ render(await api('/v1/status')); }} catch(error) {{ alert(error.message); }} }}
